...
class TestMessage(unittest.TestCase):
    def test_basic(self):
        m = Message("complete_nick!foobar", "channel_name", "message contents")
        t = Timestamp()
        self.assertTrue(m.time < t)
        self.assertEquals(m.nick_full, "complete_nick!foobar")
        self.assertEquals(m.nick_full, "complete_nick!foobar")
        self.assertEquals(m.nick, "complete_nick")
        self.assertEquals(m.chan, "channel_name")
        self.assertEquals(m.msg, "message contents")
        self.assertTrue(m.target is None)

    def test_target(self):
        m = Message("complete_nick!foobar", "channel_name", "BaZ: message contents")
        self.assertEquals(m.target, "BaZ")

    def test_serialize(self):
        m = Message("complete_nick!foobar", "channel_name", "BaZ: message contents", 10)
        self.assertEquals(m.serialize(), "10 @@@ complete_nick!foobar @@@ channel_name @@@ BaZ: message contents")
        
    def test_deserialize(self):
        m = Message("complete_nick!foobar", "channel_name", "BaZ: message contents", 10)
        m2 = Message(serialized_str=m.serialize())
        self.assertEquals(m.serialize(), m2.serialize())
        m3 = Message(serialized_str="10 @@@ complete_nick!foobar @@@ channel_name @@@ BaZ: message contents")
